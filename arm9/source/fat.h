#ifndef __FAT_H__
#define __FAT_H__

typedef struct
{
	char long_name[34];
	char short_name[12] __attribute__ ((aligned (4)));
	int is_folder;
} entry_names_t;

typedef enum
{
	SHORT_NAME = 0,
	LONG_NAME = 1
} SEARCH_TYPE;

PUT_IN_VRAM int strlen( char* str );
PUT_IN_VRAM uint32_t get_cluster_fat_value_simple(uint32_t cluster);
PUT_IN_VRAM uint32_t get_entrys_first_cluster(dir_entry_t* dir_entry);
PUT_IN_VRAM void store_long_name_part(uint8_t* buffer, dir_entry_t* cur_dir_entry, int position);

PUT_IN_VRAM void get_full_long_name(uint32_t, const char*, uint8_t*);
PUT_IN_VRAM int gen_short_name(uint8_t*, uint8_t*, uint32_t);
PUT_IN_VRAM int write_entries_to_sd(const uint8_t*, const uint8_t*, const int, uint8_t, uint32_t);
PUT_IN_VRAM void find_dir_entry(uint32_t, const char*, dir_entry_t*, SEARCH_TYPE);

#endif 		//__FAT_H__